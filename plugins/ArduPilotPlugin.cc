...
void ArduPilotPlugin::ReceiveMotorCommand()
{
    ...
    else
    {
        ...
        double now = this->dataPtr->model->GetWorld()->GetSimTime().Double();
        float pwms[MAX_MOTORS] = {0};
        float pwms2[MAX_MOTORS] = {0};
        ...
        for (unsigned i = 0; i < this->dataPtr->controls.size(); ++i)
        {
            if (i < MAX_MOTORS)
            {
                if (this->dataPtr->controls[i].channel < recvChannels)
                {
                    ...
                    //@@
                    //            printf("$$ motor%d %.2f, cmd %.2f [eul %.2f, off %.2f] %f\n", this->dataPtr->controls[i].channel, pkt.motorSpeed[this->dataPtr->controls[i].channel], cmd, this->dataPtr->controls[i].multiplier, this->dataPtr->controls[i].offset, this->dataPtr->controls[i].cmd);
                    ...
                    if (cmd_in != this->dataPtr->last_cmd)
                    {
                        ...
                        */
                        pwms[i] = pkt.motorSpeed[this->dataPtr->controls[i].channel];
                        pwms2[i] = this->dataPtr->controls[i].cmd;
                        //@@----------

                        dataPtr->last_cmd = cmd_in;
                    }
        ...
        //@@
        if(int(now * 1000) % 1000 == 0) {
            gzdbg << "PWM [" << pwms[0] << " " << pwms[1] << " " << pwms[2] << " " << pwms[3]  << "] to [" <<  pwms2[0] << " " << pwms2[1] << " " << pwms2[2] << " " << pwms2[3] << "]\n";
        }
    }
}
