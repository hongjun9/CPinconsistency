...
def input_loop():
    ...
    ### ardupilot copter
    #scriptfile = "/home/apm/ardupilot/Tools/autotest/fuzz/script.scr"   #cpi autotest scirpt
    ### PX4
    ### APMRover2

    # cpi_ext autotest script 
    scriptfile = "/home/apm/ardupilot/Tools/autotest/cpii/script.scr"   
    #time.sleep(10)
    # check gazebo state
    gazebo_pid = None
    gazebo_initiated = False
    while gazebo_initiated == False:
        time.sleep(1)
        print "Waiting Gazebo..."
        if gazebo_pid == None:
            plist = subprocess.Popen(['ps', '-A'], stdout=subprocess.PIPE)
            out, err = plist.communicate() 
            for line in out.splitlines():
                if 'gzserver' in line:
                    print "Gazebo (server) Initiated!!" 
                    gazebo_pid = int(line.split(None, 1)[0])
                    gazebo_initiated = True;
                    break
    time.sleep(10)
    ...
    ## ardu rover
    #time.sleep(15)
    #line = "wp load fuzz/rover_forward.txt"
    #print "Run " + line
    ...
    #time.sleep(5)
    #line = "script " + scriptfile
    #mpstate.input_queue.put(line)

    #while (mpstate.status.have_gps_lock != True):
    #    time.sleep(1)
    #    print("gps_lock waiting...")
    #print("GPS locked!!!") 
